...
namespace gnss_comm
{
    ...
    *-----------------------------------------------------------------------------*/
    Eigen::Matrix<double, 7, 1> psr_pos(const std::vector<ObsPtr> &obs, 
        const std::vector<EphemBasePtr> &ephems, const std::map<uint32_t, std::vector<double>> &iono_params, std::vector<size_t>& inliners,
        bool isFinePose = true, bool useDualFreq = false, const Eigen::Matrix<double, 7, 1>& last_state = Eigen::Matrix<double, 7, 1>(), 
        const std::set<int> excludeSats = std::set<int>(), const std::string& ins_name="");
    ...
    *----------------------------------------------------------------------------------------------------*/
    ...
    inline std::map<uint32_t, std::vector<double> > getDefaultIonos(){
        std::map<uint32_t, std::vector<double> > ionos;
        std::vector<double> default_iono = {0.1118E-07,  0.2235E-07, -0.4172E-06,  0.6557E-06, 0.1249E+06, -0.4424E+06,  0.1507E+07, -0.2621E+06};
        ionos[SYS_NONE] = default_iono;
        ionos[SYS_GPS] = default_iono;
        ionos[SYS_BDS] = default_iono;
        ionos[SYS_GAL] = default_iono;
        ionos[SYS_GLO] = default_iono;
        ionos[SYS_IRN] = default_iono;
        return ionos;
    }

...
namespace gnss_comm
{
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    inline bool valid_ecef(const Eigen::Vector3d& ecef){
        return !ecef.hasNaN() &&  ecef.norm() > EARTH_MEAN_RADIUS - 1000.0 && ecef.norm() < EARTH_MEAN_RADIUS + 10000.0;
    }
    cv::Mat drawTopSatView(const std::vector<ObsPtr>& valid_meas, const std::vector<EphemBasePtr>& valid_ephems, const std::vector<size_t>& inliners, Eigen::Vector3d coarse_pos_ecef, bool bShow = true, const std::string name = "", const int N_ALL = -1);
    cv::Mat drawSrnView(const std::vector<ObsPtr>& valid_meas, const std::vector<size_t>& inliners, bool bShow = true, const std::string name = "", const int N_ALL = -1);
    void grabObsNum(double time, const int n_obs_all, const int n_obs_valid, const int n_obs_spp = 0, const double time_gap = 0.1);
    cv::Mat drawObsNumView(bool bShow = true, const std::string name = "");
    cv::Mat monitorGnssMeas(int N_ALL_Obs, const std::vector<ObsPtr>& valid_meas, const std::vector<size_t>& inliners, std::pair<int, int> RTK_SPP_Status, const double time_gap=0.1, bool bShow = true, const std::string name = "");

...
namespace gnss_comm
{
    ...
    *-----------------------------------------------------------------------------*/
    ...
    Eigen::Vector3d eph2pos(const gtime_t &curr_time, const EphemPtr ephem_ptr, double *svdt)
    {
        ...
        double toe_tow = ephem_ptr->toe_tow;

        if (sys == SYS_BDS && (prn <= 5 || prn >= 59))     // BDS GEO satellite
        {
    ...
    *-----------------------------------------------------------------------------*/
    ...
    cv::Vec3b hsvToRgb(const double h) {
        float s = 1.0; // 饱和度
        float v = 1.0; // 明度
        ...
        switch (i % 6) {
            case 0: return cv::Vec3b(static_cast<uchar>(v * 255), static_cast<uchar>(t * 255), static_cast<uchar>(p * 255));
            case 1: return cv::Vec3b(static_cast<uchar>(q * 255), static_cast<uchar>(v * 255), static_cast<uchar>(p * 255));
            case 2: return cv::Vec3b(static_cast<uchar>(p * 255), static_cast<uchar>(v * 255), static_cast<uchar>(t * 255));
            case 3: return cv::Vec3b(static_cast<uchar>(p * 255), static_cast<uchar>(q * 255), static_cast<uchar>(v * 255));
            case 4: return cv::Vec3b(static_cast<uchar>(t * 255), static_cast<uchar>(p * 255), static_cast<uchar>(v * 255));
            case 5: return cv::Vec3b(static_cast<uchar>(v * 255), static_cast<uchar>(p * 255), static_cast<uchar>(q * 255));
            default: return cv::Vec3b(0, 0, 0); // 应确保不会到达这里
        }
    }

    cv::Vec3b srn2color(double srn){ double r=(srn-25)/15.0; if(r<0)r=0; if(r>1)r=1; return hsvToRgb(0.67*(r));};
    cv::Mat drawTopSatView(const std::vector<ObsPtr>& valid_meas, const std::vector<EphemBasePtr>& valid_ephems, 
        const std::vector<size_t>& inliners, Eigen::Vector3d coarse_pos_ecef, bool bShow, const std::string name, const int N_ALL){

        std::set<ObsPtr> inliner_meas;
        for(auto& i : inliners) inliner_meas.emplace(valid_meas[i]);


        const int N_WIDTH = 400;
        const int Length = N_WIDTH / 2.0;
        cv::Mat topView(cv::Size(N_WIDTH,N_WIDTH), CV_8UC3, cv::Scalar(0,0,0));
        for(int i=0; i<=90; i+=15)
            cv::circle(topView, cv::Point2f(Length, Length), Length * sin(EIGEN_PI/180.0*i), cv::Scalar(155, 155, 155), 1);
        auto azel2uv = [Length](double* azel){ 
            double lxy = Length * cos(azel[1]);
            double lx = lxy * sin(azel[0]);
            double ly = lxy * cos(azel[0]);
            return cv::Point2f(Length+lx,Length-ly);
        };
        for(int i=0; i<=10; ++i){
            cv::circle(topView, cv::Point2f(5+i*12, 5), 5, cv::Scalar(hsvToRgb(i/10.0*0.67)) , cv::FILLED);
        }

        std::string showText = std::to_string(valid_meas.size()) + "-" + std::to_string(inliners.size());
        if(N_ALL >= (int)valid_meas.size()) showText = std::to_string(N_ALL) + "-" + showText;
        cv::putText(topView, showText, cv::Point(20,40), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255,255,255), 1);

        if(!valid_ecef(coarse_pos_ecef)) return topView;
        
        for(int i=0; i<valid_meas.size(); ++i){
            const ObsPtr& obs = valid_meas[i]; 
            uint32_t sys = satsys(obs->sat, NULL);
            Eigen::Vector3d sat_ecef;
            if (sys == SYS_GLO)
                sat_ecef = geph2pos(obs->time, std::dynamic_pointer_cast<GloEphem>(valid_ephems[i]), NULL);
            else
                sat_ecef = eph2pos(obs->time, std::dynamic_pointer_cast<Ephem>(valid_ephems[i]), NULL);
            double azel[2] = {0, M_PI/2.0};
            sat_azel(coarse_pos_ecef, sat_ecef, azel);
            
            bool isFine = inliner_meas.count(obs) > 0;
            
            auto pt =  azel2uv(azel);
            cv::circle(topView, pt, 5, cv::Scalar(srn2color(obs->CN0[0])) , cv::FILLED);
            cv::putText(topView, gnss_comm::sat2str(obs->sat), cv::Point(pt.x+9,pt.y), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255,255,255), 1);
            if(isFine) cv::circle(topView, azel2uv(azel), 7, cv::Scalar(255, 255, 255) , 2);
        }
        
        if(bShow){
            cv::imshow(name + " TopView", topView);
            cv::waitKey(2);
        }
    ...
    cv::Mat drawSrnView(const std::vector<ObsPtr>& valid_meas, const std::vector<size_t>& inliners, bool bShow, const std::string name, const int N_ALL){
        const int GAP = 10;
        const int W_L1 = 16;
        const int W_L5 = 8;
        int image_width = (valid_meas.size()+1) * (W_L1 + GAP);
        const int W = image_width > 600 ? image_width : 600;
        const int H = 400;
        const int H_bottom = 400 - 60;

        auto start_x = [&GAP, &W_L1](int i){ return W_L1*(i+1)+GAP*i; };
        auto H_top = [&H_bottom](double srn){return H_bottom - srn*5; };
        auto color_L1 = [](bool bInliner){return bInliner ? cv::Scalar(255,0,0) : cv::Scalar(100,100,100);};
        auto color_L5 = [](bool bInliner){return bInliner ? cv::Scalar(0,255,0) : cv::Scalar(200,200,200);};

        std::set<ObsPtr> inliner_meas;
        for(auto& i : inliners) inliner_meas.emplace(valid_meas[i]);
        if(inliner_meas.empty()) inliner_meas.insert(valid_meas.begin(), valid_meas.end());

        cv::Mat view(cv::Size(W, H), CV_8UC3, cv::Scalar(0,0,0));
        for(int i=0; i<valid_meas.size(); ++i){
            auto& m = valid_meas[i];
            int l1_idx = -1; L1_freq(m, &l1_idx);
            if(l1_idx < 0) continue;
            bool isFine = inliner_meas.count(m) > 0;
            int x_pt = start_x(i);
            cv::line(view, cv::Point2i(x_pt, H_bottom), cv::Point2i(x_pt, H_top(m->CN0[l1_idx])), color_L1(isFine), W_L1);
            if(m->CN0.size() >= 2){
                int l5_idx = l1_idx == 0 ? 1 : 0;
                cv::line(view, cv::Point2i(x_pt, H_bottom), cv::Point2i(x_pt, H_top(m->CN0[l5_idx])), color_L5(isFine), W_L5);
            }
            cv::putText(view,gnss_comm::sat2str(m->sat), cv::Point2i(x_pt-W_L1/2,H_bottom+20), cv::FONT_HERSHEY_SIMPLEX, 0.35, cv::Scalar(255,255,255), 1);

            cv::circle(view, cv::Point2i(x_pt,H_bottom+30), 5, cv::Scalar(srn2color(m->CN0[0])) , cv::FILLED);
        }

        cv::line(view, cv::Point(0, H_top(25)), cv::Point(W, H_top(25)), cv::Scalar(0,0,155));
        cv::line(view, cv::Point(0, H_top(30)), cv::Point(W, H_top(30)), cv::Scalar(155,0,0));
        cv::line(view, cv::Point(0, H_top(40)), cv::Point(W, H_top(40)), cv::Scalar(0,155,0));

        std::string showText = std::to_string(valid_meas.size()) + "-" + std::to_string(inliners.size());
        if(N_ALL >= (int)valid_meas.size()) showText = std::to_string(N_ALL) + "-" + showText;
        cv::putText(view, showText, cv::Point(20,40), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255,255,255), 1);

        if(bShow){
            cv::imshow(name + " SrnView", view);
            cv::waitKey(2);
        }
    ...
    std::vector<int> n_obs_alls, n_obs_valids, n_obs_spps;
    std::vector<double> t_obs;
    void grabObsNum(double time, const int n_obs_all, const int n_obs_valid, const int n_obs_spp, const double time_gap){
        double last_time = t_obs.empty() ? time : t_obs.back();
        double n = (time - last_time) / time_gap - 0.5; // 向下取整
        for(int i=0; i < int(n); ++i){
            n_obs_alls.emplace_back(0);
            n_obs_valids.emplace_back(0);
            n_obs_spps.emplace_back(0);
        }
        n_obs_alls.emplace_back(n_obs_all);
        n_obs_valids.emplace_back(n_obs_valid);
        n_obs_spps.emplace_back(n_obs_spp);
        t_obs.emplace_back(time);
        // if(n_obs_alls.size() < 3) return cv::Mat();
    }
    cv::Mat drawObsNumView(bool bShow, const std::string name){
        ...
        const int N_Max = W / W_Line;
        const int s = n_obs_alls.size() > N_Max ? n_obs_alls.size() - N_Max : 0;
        auto drawH = [&H](int n){return H-n*8;};
        auto drawPt = [&s, &W_Line, &drawH](int i, int n){return cv::Point((i-s)*W_Line, drawH(n)); };
        
        // for(int i=s; i<n_obs_alls.size(); ++i) cv::rectangle(view, cv::Rect2i((i-s)*W_Line, drawH(n_obs_alls[i]), W_Line, H_Line), cv::Scalar(0,0,255), cv::FILLED);
        // for(int i=s; i<n_obs_valids.size(); ++i) cv::rectangle(view, cv::Rect2i((i-s)*W_Line, drawH(n_obs_valids[i]), W_Line, H_Line), cv::Scalar(255,0,0), cv::FILLED);
        // for(int i=s; i<n_obs_spps.size(); ++i) cv::rectangle(view, cv::Rect2i((i-s)*W_Line, drawH(n_obs_spps[i]), W_Line, H_Line), cv::Scalar(0,255,0), cv::FILLED);

        for(int i=s+1; i<n_obs_alls.size(); ++i) cv::line(view, drawPt(i-1, n_obs_alls[i-1]), drawPt(i, n_obs_alls[i]),  cv::Scalar(0,0,255), H_Line);
        for(int i=s+1; i<n_obs_valids.size(); ++i) cv::rectangle(view, drawPt(i-1, n_obs_valids[i-1]), drawPt(i, n_obs_valids[i]), cv::Scalar(255,0,0), H_Line);
        for(int i=s+1; i<n_obs_spps.size(); ++i) cv::rectangle(view, drawPt(i-1, n_obs_spps[i-1]), drawPt(i, n_obs_spps[i]), cv::Scalar(0,255,0), H_Line);

        cv::line(view, cv::Point(0, drawH(10)), cv::Point(W, drawH(10)), hsvToRgb(10.0/60.0)*0.4);
        cv::line(view, cv::Point(0, drawH(20)), cv::Point(W, drawH(20)), hsvToRgb(20.0/60.0)*0.4);
        cv::line(view, cv::Point(0, drawH(30)), cv::Point(W, drawH(30)), hsvToRgb(30.0/60.0)*0.4);
        cv::line(view, cv::Point(0, drawH(40)), cv::Point(W, drawH(40)), hsvToRgb(40.0/60.0)*0.4);

        if(bShow){
            cv::imshow(name + " ObsView", view);
            cv::waitKey(2);
        }
    ...
    cv::Mat monitorGnssMeas(int N_ALL_Obs, const std::vector<ObsPtr>& valid_meas, const std::vector<size_t>& inliners, std::pair<int, int> RTK_SPP_Status, const double time_gap, bool bShow, const std::string name){
        if(valid_meas.empty()) return cv::Mat();
        double curTime =  gnss_comm::time2sec(valid_meas.front()->time);
        cv::Mat numObsView = drawObsNumView(false, name);
        cv::Mat srnView = drawSrnView(valid_meas, inliners, false, name, N_ALL_Obs);
        std::vector<cv::Scalar> colors = {cv::Scalar(50,50,50), cv::Scalar(255,0,0), cv::Scalar(0,255,0)};
        cv::circle(numObsView, cv::Point(20,20), 8, colors[RTK_SPP_Status.first], cv::FILLED); // 0:invalid; 1:not fixed; 2:fixed;
        cv::circle(numObsView, cv::Point(40,20), 8, colors[RTK_SPP_Status.second+1], cv::FILLED); // 0:fail;  1:suc;
        cv::Mat concatView;
        cv::hconcat(numObsView, srnView, concatView);
        if(bShow){
            cv::imshow(name + " Monitor", concatView);
            cv::waitKey(2);
        }
        return concatView;
    }
